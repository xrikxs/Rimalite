...
<script type="text/javascript" src="templates/orderforms/{$carttpl}/js/pwstrength.js"></script>
...
<div class="cartbuttons"><input type="button" class="btn btn-danger" value="{$LANG.emptycart}" onclick="emptyCart();return false" /> <input type="button" class="btn btn-success" value="{$LANG.continueshopping}" onclick="window.location='cart.php'" /></div>
...
<tr><td class="fieldlabel">{$LANG.clientareaemail}</td><td class="fieldarea"><input type="text" class="form-control" name="loginemail" size="40" /></td></tr>
<tr><td class="fieldlabel">{$LANG.clientareapassword}</td><td class="fieldarea"><input type="password" class="form-control" name="loginpw" size="40" /></td></tr>
</table>
</div>
<div class="signupfields{if $custtype eq "existing" && !$loggedin} hidden{/if}" id="signupfrm">
<table width="100%" cellspacing="0" cellpadding="0" class="configtable">
<tr><td class="fieldlabel">{$LANG.clientareafirstname}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.firstname}{else}<input type="text" class="form-control" name="firstname" tabindex="1" value="{$clientsdetails.firstname}" />{/if}</td><td class="fieldlabel">{$LANG.clientareaaddress1}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.address1}{else}<input type="text" class="form-control" name="address1" tabindex="7" value="{$clientsdetails.address1}" />{/if}</td></tr>
<tr><td class="fieldlabel">{$LANG.clientarealastname}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.lastname}{else}<input type="text" class="form-control" name="lastname" tabindex="2" value="{$clientsdetails.lastname}" />{/if}</td><td class="fieldlabel">{$LANG.clientareaaddress2}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.address2}{else}<input type="text" class="form-control" name="address2" tabindex="8" value="{$clientsdetails.address2}" />{/if}</td></tr>
<tr><td class="fieldlabel">{$LANG.clientareacompanyname}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.companyname}{else}<input type="text" class="form-control" name="companyname" tabindex="3" value="{$clientsdetails.companyname}" />{/if}</td><td class="fieldlabel">{$LANG.clientareacity}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.city}{else}<input type="text" class="form-control" name="city" tabindex="9"  value="{$clientsdetails.city}" />{/if}</td></tr>
<tr><td class="fieldlabel">{$LANG.clientareaemail}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.email}{else}<input type="text" class="form-control" name="email" tabindex="4" value="{$clientsdetails.email}" />{/if}</td><td class="fieldlabel">{$LANG.clientareastate}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.state}{else}<input type="text" class="form-control" name="state" tabindex="10" value="{$clientsdetails.state}" />{/if}</td></tr>
<tr>{if !$loggedin}<td class="fieldlabel">{$LANG.clientareapassword}</td><td class="fieldarea"><input type="password" class="form-control" name="password" tabindex="5" id="newpw" size="20" value="{$password}" /></td>{else}<td class="fieldlabel"></td><td class="fieldarea"></td>{/if}<td class="fieldlabel">{$LANG.clientareapostcode}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.postcode}{else}<input type="text" class="form-control" name="postcode" tabindex="11" size="15" value="{$clientsdetails.postcode}" />{/if}</td></tr>
<tr>{if !$loggedin}<td class="fieldlabel">{$LANG.clientareaconfirmpassword}</td><td class="fieldarea"><input type="password" name="password2" class="form-control" tabindex="6" size="20" value="{$password2}" /></td>{else}<td class="fieldlabel"></td><td class="fieldarea"></td>{/if}<td class="fieldlabel">{$LANG.clientareacountry}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.country}{else}{$clientcountrydropdown|replace:'<select':'<select tabindex="12"'}{/if} </td></tr>
<tr><td colspan="2" class="fieldarea">{if !$loggedin}<script language="javascript">showStrengthBar();</script>{/if}</td><td class="fieldlabel">{$LANG.clientareaphonenumber}</td><td class="fieldarea">{if $loggedin}{$clientsdetails.phonenumber}{else}<input type="text" class="form-control" name="phonenumber" tabindex="13" size="20" value="{$clientsdetails.phonenumber}" />{/if}</td></tr>
...
<table width="100%" cellspacing="0" cellpadding="0" class="configtable">
<tr><td class="fieldlabel">{$LANG.clientareafirstname}</td><td class="fieldarea"><input type="text" class="form-control"  name="domaincontactfirstname" value="{$domaincontact.firstname}" /></td><td class="fieldlabel">{$LANG.clientareaaddress1}</td><td class="fieldarea"><input type="text" class="form-control" name="domaincontactaddress1" style="width:80%;" value="{$domaincontact.address1}" /></td></tr>
<tr><td class="fieldlabel">{$LANG.clientarealastname}</td><td class="fieldarea"><input type="text" class="form-control"  name="domaincontactlastname" value="{$domaincontact.lastname}" /></td><td class="fieldlabel">{$LANG.clientareaaddress2}</td><td class="fieldarea"><input type="text" class="form-control" name="domaincontactaddress2" style="width:80%;" value="{$domaincontact.address2}" /></td></tr>
<tr><td class="fieldlabel">{$LANG.clientareacompanyname}</td><td class="fieldarea"><input type="text"  class="form-control" name="domaincontactcompanyname"" value="{$domaincontact.companyname}" /></td><td class="fieldlabel">{$LANG.clientareacity}</td><td class="fieldarea"><input type="text" class="form-control" name="domaincontactcity" style="width:80%;" value="{$domaincontact.city}" /></td></tr>
<tr><td class="fieldlabel">{$LANG.clientareaemail}</td><td class="fieldarea"><input type="text" class="form-control" name="domaincontactemail" value="{$domaincontact.email}" /></td><td class="fieldlabel">{$LANG.clientareastate}</td><td class="fieldarea"><input type="text" class="form-control" name="domaincontactstate" value="{$domaincontact.state}" /></td></tr>
<tr><td class="fieldlabel">{$LANG.clientareaphonenumber}</td><td class="fieldarea"><input type="text" class="form-control"  name="domaincontactphonenumber" size="20" value="{$domaincontact.phonenumber}" /></td><td class="fieldlabel">{$LANG.clientareapostcode}</td><td class="fieldarea"><input type="text" class="form-control" name="domaincontactpostcode" size="15" value="{$domaincontact.postcode}" /></td></tr>
...
{if $promotioncode}{$promotioncode} - {$promotiondescription}<br /><a href="{$smarty.server.PHP_SELF}?a=removepromo">{$LANG.orderdontusepromo}</a>{else}<div class="form-inline"><div class="form-group couponbox"><input type="text" class="form-control" name="promocode" size="20" value="" /></div> <input type="submit" name="validatepromo" class="btn btn-primary" value="{$LANG.orderpromovalidatebutton}" /></div>{/if}
</div>

{if $shownotesfield}
<div class="signupfields padded">
<h2>{$LANG.ordernotes}</h2>
<textarea name="notes" class="form-control" rows="2" style="width:100%" onFocus="if(this.value=='{$LANG.ordernotesdescription}'){ldelim}this.value='';{rdelim}" onBlur="if (this.value==''){ldelim}this.value='{$LANG.ordernotesdescription}';{rdelim}">{$notes}</textarea>
</div>
{/if}
